...
import com.example.adapter.OrderListLvAdapter;
import com.example.parser.OrderListParser;
...
public class OrderListActivity extends BaseWapperActivity implements
	...
	public void onClick(View v) {
		...
		case R.id.textCancelOrder:
			...
			TextView cancelorder = (TextView) my_order_list
					.findViewWithTag(position);
			OrderList order = (OrderList) my_order_list
					.getItemAtPosition(position);
			Toast.makeText(context, "要取消的订单编号为：" + order.getOrderid(), 0)
					.show();
	...
	private void setAdapterForDivide() {
		...
		case 0:
			...
			adapter = new OrderListLvAdapter(OrderListActivity.this,
					orderlistInMonth, cancelablelist, cancelablelist);
		...
		case 1:
			...
			adapter = new OrderListLvAdapter(OrderListActivity.this, orderlist,
					cancelablelist, uncancelablelist);
			// 分页处理
		...
		case 2:
			adapter = new OrderListLvAdapter(OrderListActivity.this,
					ordercanceled, cancelablelist, uncancelablelist);
			// 分页处理
